# React Three Fiber 避難シミュレーション

React Three Fiber と AgentScript を使用したリアルタイム避難シミュレーションプロジェクト。Web ブラウザ上で動作する 3D エージェントベースモデル（ABM）による群衆避難の可視化・分析システムです。

## 📋 概要

このプロジェクトは、建物内の避難行動をリアルタイムで可視化するシミュレーションアプリケーションです。AgentScript による数理モデルと React Three Fiber による 3D レンダリングを組み合わせ、技術者・研究者向けの避難解析ツールとして設計されています。
